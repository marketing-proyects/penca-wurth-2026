...
# --- 2. FIXTURE COMPLETO (72 Partidos - Grupos A al L) ---
def cargar_fixture():
    # Generamos los 72 partidos de la fase de grupos
    data = [
        # GRUPO A
        ...
        # ... El sistema soporta los 72 partidos cargados aquí
    ]
    
    # Estructura de Eliminatorias (Octavos a Final)
    eliminatorias = [
...
# --- 4. RENDER CABECERA (LOGO RECUPERADO) ---
st.markdown('<div class="logo-box">', unsafe_allow_html=True)
# Intenta cargar local, si no, usa la URL de respaldo
...
with menu[0]:
    ...
    u_sec = c4.selectbox("Sector:", ["Ventas", "Logística", "IT", "Marketing", "Administración", "RRHH", "Otros"])

    if u_nom and u_ape and u_wn:
        # Recuperar datos de SQLite
        db_conn = sqlite3.connect('penca.db')
        df_u = pd.read_sql(f"SELECT * FROM apuestas WHERE wn='{u_wn}'", db_conn)
        db_conn.close()

        # Comodín Ventas (Pop-up simplificado si es nuevo)
        v_com_prev = 100.0
        if not df_u.empty:
            p_com = df_u[df_u['partido_id'] == 999]
            if not p_com.empty: v_com_prev = float(p_com.iloc[0]['g1'])
        
        st.info(f"🃏 **Comodín de Ventas:** Tu apuesta actual es del {v_com_prev}%")
        ...
        with f_tabs[0]:
            ...
            with st.form("penca_form_sqlite"):
                for i, dia in enumerate(dias):
                    with tabs_dias[i]:
                        ...
                        for _, row in partidos_dia.iterrows():
                            ...
                            st.markdown(f'<div class="grupo-header-card"><span>GRUPO {row["grupo"]} {check}</span><span style="font-size: 13px; opacity: 0.8;">{row["hora"]} hs</span></div>', unsafe_allow_html=True)
                ...
                # Input del Comodín al final del form
                u_com = st.number_input("¿% Cumplimiento empresa Junio?", 0.0, 200.0, v_com_prev, step=0.1)
                ...
                if st.form_submit_button("💾 GUARDAR TODOS LOS PRONÓSTICOS"):
                    ...
                    c.execute("INSERT INTO apuestas VALUES (?,?,?,?,?,?,?,?)", 
                             (u_wn, u_nom, u_ape, u_sec, 999, u_com, 0, datetime.now().strftime("%Y-%m-%d %H:%M")))
                    db_conn.commit()
                    db_conn.close()
                    st.success("¡Datos guardados localmente!")
                    st.rerun()

# --- TAB 4: ADMIN (Clave: wurth2026) ---
with menu[3]:
    ...
    if clave == "wurth2026":
        ...
        if not df_admin.empty:
            st.write(f"Participantes activos: {df_admin['wn'].nunique()}")
            csv = df_admin.to_csv(index=False).encode('utf-8')
            st.download_button("📥 DESCARGAR EXCEL (CSV)", csv, "penca_wurth_2026.csv", "text/csv")
